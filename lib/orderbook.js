...
class Orderbook {
  constructor() {
    this._ordersByID = {};
    this._bids = new RBTree((a, b) => b.price.cmp(a.price)); // high to low
    this._asks = new RBTree((a, b) => a.price.cmp(b.price)); // low to high
  }
  ...
  estimateMatchCost(params) {
    let size = num(params.size);
    const tree = this._getTree(params.side);
    let iterator = tree.iterator();
    let cost = num(0);
    while(size.gt(0)) {
      let node = iterator.next();
      for (let order in node.orders) {
        let takeSize = order.size.gte(size) ? size : order.size;
        size = size.sub(takeSize);
        cost.add(takeSize.mul(order.price));
      }
    }
    return cost;
  }
